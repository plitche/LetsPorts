...
public class TrainerListCommand implements CommonVoidCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		totalRecord = trainerClassDao.trainerClassCount();
		endRecord = endRecord < totalRecord ? endRecord : totalRecord;
		paging = Paging.getPaging("TrainerClassList.leo", totalRecord, recordPerPage, page);
		// 모든 클래스목록들을 뿌려주기 위한 작업
		model.addAttribute("MeetingList", trainerClassDao.trainerList(beginRecord, endRecord));
		model.addAttribute("ClassTags", trainerClassDao.classTag());		
		model.addAttribute("totalRecord", totalRecord);
		model.addAttribute("paging", paging);
		model.addAttribute("page", page);
		model.addAttribute("recordPerPage", recordPerPage);
		model.addAttribute("Lists", 1);
	}
