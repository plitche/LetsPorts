...
public class Tag6Command implements CommonVoidCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		String 댄스 = request.getParameter("댄스");
		...
		totalRecord = trainerClassDao.댄스TagCount();
		endRecord = endRecord < totalRecord ? endRecord : totalRecord;
		paging = Paging.getPaging("Tag6.leo?댄스=" + 댄스, totalRecord, recordPerPage, page);
		...
		model.addAttribute("MeetingList", trainerClassDao.Tag0(beginRecord, endRecord, 댄스));
		model.addAttribute("Lists", 16);
		model.addAttribute("Tag6", "#" + 댄스);
